...
import           Elescore.Projection           (DisruptionProjection,
                                                DisruptionsPerDay, Facilities,
                                                Objects, SumOfDowntimes,
...
data Env = Env
  ...
  , envDisruptionsPerDay :: IORef DisruptionsPerDay
...
mkEnv c = do
  ...
  dissDay <- newIORef mempty
  dtimes <- newIORef mempty
  objs <- newIORef mempty
  fs <- newIORef mempty
  return $ Env c mgr conn diss dissDay dtimes objs fs
...
disruptionsPerDay :: Elescore (IORef DisruptionsPerDay)
disruptionsPerDay = Elescore $ asks envDisruptionsPerDay
